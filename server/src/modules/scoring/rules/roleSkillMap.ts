...
export const ROLE_SKILL_REQUIREMENTS: Record<string, string[]> = {
...
export function getRequiredSkillsForRole(targetRole: string | undefined): string[] {
  if (!targetRole) return DEFAULT_SKILL_REQUIREMENTS;
  const normalized = targetRole.toLowerCase();
  const key = Object.keys(ROLE_SKILL_REQUIREMENTS).find((role) =>
    normalized.includes(role) || role.includes(normalized)
  );
  return key ? ROLE_SKILL_REQUIREMENTS[key] : DEFAULT_SKILL_REQUIREMENTS;
}
